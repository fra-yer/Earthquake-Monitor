...
This distinction is useful because the geodetically correct great-circle bearings become increasingly unintuitive the farther away an event occurs. For example, the great-circle bearing from Germany to Tonga in the South Pacific genuinely starts toward the north-east. The great-circle route first curves up over Northern Europe and crosses the Arctic, before turning south through the Pacific. This is the shortest path *on the sphere*, but deeply counterintuitive when you think in flat-map terms, where Tonga lies to the south-east of Germany (see [this image](https://github.com/fra-yer/Earthquake-Monitor/blob/main/docs/great-circle%20image.jpg) to see the difference). Importantly, the bearing the (longer) straight-line path is constant, but permanently changing for the great-circle path. The same effect is seen with transatlantic flights from Europe to the US East Coast that appear to arc northward on a flat map, but actually follow the shortest route on the globe (and the pilots must regularly adjust the direction of the plane to stay on this route).
